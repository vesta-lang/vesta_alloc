 ...
 * @brief Three ways to copy, side by side, on every single row.
 *
 * THE THREE COLUMNS, and why they are all here rather than in an appendix:
 *
 *   C      `vesta_memcpy(d, s, N)` -- the byte-counting interface.  Knows the
 *          length, knows nothing about the alignment of the destination, so it
 *          emits the prologue that aligns it at run time.
 *   C++    `util::vesta_memcopy(dst, src)` -- the typed one.  Gets both the
 *          length and the alignment from the type, so where the type is
 *          over-aligned the prologue is not emitted at all.
 *   libc   the system's `memcpy`, as the reference.
 *
 * Comparing only two of them answers half the question.  The interesting part
 * is not "are we faster than libc" but "what does the type buy", and that only
 * shows when the same size and the same addresses go through all three.
 *
 * WHY THE SIZE IS A TEMPLATE PARAMETER.  Because the typed interface needs a
 * type, and a type needs a size at compile time.  It also means the C column
 * gets a constant length too, which is the fair comparison: same information,
 * different interface.
 *
 * WHAT THE ROWS COVER, and why each one:
 *
 *   1. aligned     the type really is 32-byte aligned.  This is where the
 *                  typed interface can drop the prologue.
 *   2. unaligned   the type declares alignment 1 and the buffers sit at an odd
 *                  offset.  Here the typed one CANNOT drop anything, and the
 *                  point of the row is precisely that it does not pretend to:
 *                  it has to come out level with the C one.
 *   3. scattered   over 16 MiB, not in cache.  The realistic case for an
 *                  allocator, where the block being copied has not been
 *                  touched for a while.
 *
 * SIZES INCLUDE NON-MULTIPLES on purpose.  The tail is where this kind of code
 * gets it wrong -- one byte too many or too few -- and where an implementation
 * can look fine on multiples of 32 and fall apart on everything else.  The ones
 * just under a power of two (15, 4095) are the worst case for the cascades of
 * overlapping blocks.
 *
 * INTERLEAVED, AND IN BOTH ORDERS.  Every measurement runs A-B-C-C-B-A and
 * averages each side.  Two runs in a row do not distinguish a change from
 * drift: if the machine speeds up or slows down mid-benchmark, a plain
 * A-then-B-then-C blames the last one for it.
 *
 ...
 * cuando la direccion NO lo esta seria mentirle al codigo y romperlo: las
 * escrituras alineadas fallan.  Por eso los bufers de abajo se colocan a mano
 * en una direccion que cumpla lo que el tipo promete.
 */
 ...
 * entre el primer y el tercer cuartil dividido por la mediana, tomando el peor
 * de los tres --.  No es decoracion: es lo que decide si la fila puede
 * declarar un ganador.  Ver @c row.
 */
struct Trio {
 ...
 *
 * @p off desalinea a proposito en las filas que lo piden.  El bufer vive en el
 * vector que se le pasa, que es de quien llama.
 */
/**
 ...
 *
 * Con el puntero opaco -- que es como llega en la vida real, desde un asignador
 * o desde un campo de una estructura -- la version de C paga el prologo y la
 * que tiene el tipo no, porque no lo deduce del calculo: se lo dice
 * `alignof(T)`.  Esa asimetria es justo lo que se quiere medir.
 */
...
uint8_t *aligned_at(std::vector<uint8_t> &store, size_t bytes, size_t align,
    ...
    /* Opaco a proposito: ver `opaque`. */
    return opaque(reinterpret_cast<uint8_t *>(p) + off);
}

/**
 * @brief La copia del SISTEMA, alcanzada por un puntero que el compilador no
 *        puede seguir.
 *
 * Este es el UNICO sitio de todo el arbol donde se llama a la de la libreria de
 ...
 * compilador RECONOCE `memcpy` como funcion suya y la sustituye por codigo
...
[[gnu::always_inline]] inline void one(uint8_t *d, const uint8_t *s) noexcept {
    if (W == kC) {
        vesta_memcpy(d, s, N);
    } else if (W == kCpp) {
        util::vesta_memcopy(reinterpret_cast<Block<N, A> *>(d),
                            reinterpret_cast<const Block<N, A> *>(s));
    } else {
        system_copy()(d, s, N);
    }
}
 ...
 */
template <size_t N, size_t A, size_t Off, int W> double hot(int rounds) {
    std::vector<uint8_t> dstore, sstore;
    uint8_t *d = aligned_at(dstore, N, A, Off);
    const uint8_t *s = aligned_at(sstore, N, A, Off);
    for (size_t i = 0; i < N; ++i)
        const_cast<uint8_t *>(s)[i] = uint8_t(i * 31u + 7u);

    uint64_t sink = 0;
    one<N, A, W>(d, s); // una fuera de la medida: calienta y decide el despacho
    ...
    for (int i = 0; i < rounds; ++i) {
        ...
        escape(ss);
        one<N, A, W>(dd, ss);
        sink += dd[0]; // tocar el destino: la copia NO es codigo muerto
    }
...
/**
 * @brief Copias repartidas por una region de 16 MiB.
 *
 ...
 *
 * Las posiciones se redondean a @p A porque el tipo promete esa alineacion y
 * hay que cumplirsela.  Las tres columnas usan las MISMAS direcciones, que es
 * lo que hace justa la comparacion.
 */
template <size_t N, size_t A, size_t Off, int W> double scattered(int rounds) {
    ...
    uint64_t sink = 0;
    const size_t reach = (kSpan - N - 64) / 2;
    ...
    for (int i = 0; i < rounds; ++i) {
        ...
        seed ^= seed >> 7;
        seed ^= seed << 17;
        // Dos posiciones al azar que no se solapan, redondeadas a la
        // alineacion que el tipo promete.
        const size_t a = (size_t(seed >> 33) % reach) & ~(A - 1);
        const size_t b = reach + ((size_t(seed >> 11) % reach) & ~(A - 1));
        uint8_t *d = base + a;
        const uint8_t *s = base + b;
        escape(d);
        escape(s);
        one<N, A, W>(d, s);
        sink += d[0];
...
/// Ordena por insercion.  Son unas pocas decenas de elementos; cualquier cosa
/// mas elaborada aqui seria mas codigo para el mismo resultado.
 ...
 * las limpias.  Una proporcion no tiene esa propiedad -- el ruido puede caer
 * en el numerador, y sube, o en el denominador, y baja --, asi que quedarse con
 * la mitad baja no seria limpiar, seria SESGAR hacia el lado que nos perjudica
 * o nos favorece segun la fila.
 *
 ...
 * frena durante la medida, el efecto se reparte entre las tres en vez de caer
 * sobre una.
 */
template <size_t N, size_t A, size_t Off> Trio hot_trio(int rounds) {
    double c[kReps], p[kReps], l[kReps];
    for (int r = 0; r < kReps; ++r) {
        if (r & 1) {
            l[r] = hot<N, A, Off, kLibc>(rounds);
            p[r] = hot<N, A, Off, kCpp>(rounds);
            c[r] = hot<N, A, Off, kC>(rounds);
        } else {
            c[r] = hot<N, A, Off, kC>(rounds);
            p[r] = hot<N, A, Off, kCpp>(rounds);
            l[r] = hot<N, A, Off, kLibc>(rounds);
...
/// @copydoc hot_trio
template <size_t N, size_t A, size_t Off> Trio scattered_trio(int rounds) {
    double c[kReps], p[kReps], l[kReps];
    for (int r = 0; r < kReps; ++r) {
        if (r & 1) {
            l[r] = scattered<N, A, Off, kLibc>(rounds);
            p[r] = scattered<N, A, Off, kCpp>(rounds);
            c[r] = scattered<N, A, Off, kC>(rounds);
        } else {
            c[r] = scattered<N, A, Off, kC>(rounds);
            p[r] = scattered<N, A, Off, kCpp>(rounds);
            l[r] = scattered<N, A, Off, kLibc>(rounds);
 ...
 *
 * Las dos proporciones se apuntan segun salen, pero los graficos se dibujan
 * TODOS AL FINAL y no debajo de su tabla.  La razon es que asi se pueden
 * comparar entre si: la pregunta interesante casi nunca es como se comporta una
 * seccion, es en que se diferencia de la de al lado -- que le pasa a lo mismo
 * cuando la alineacion es 64 en vez de 32, o cuando el bloque no esta en cache
 * --.  Con cada dibujo pegado a su tabla, esa comparacion exige recordar la
 * silueta anterior mientras se leen cuarenta numeros por medio.
 */
struct Plot {
    const char *name = "";
    /// En que pasada se midio.  En un procesador hibrido la misma seccion se
    /// mide una vez por clase de nucleo, y sin esto los ocho dibujos se
    /// convertirian en veinticuatro sin decir cual es cual.
    const char *pass = "";
     ...
     * COMO ESCALA cada uno.  Una proporcion de 1,00x no distingue entre las tres
     * yendo rapido y las tres yendo mal. */
    std::vector<double> t_c, t_cpp, t_libc; ///< ns por operacion
    std::vector<double> bytes;              ///< cuantos bytes movio cada fila
 ...
 *
 * @param name Que varia en esta tabla.  Casi siempre el tamano, pero las
 *             tablas de barrido varian la alineacion o el desplazamiento y el
 *             tamano se queda fijo, asi que la etiqueta se pasa aparte.
 * @param n    Bytes movidos, que NO se deduce de la etiqueta y hace falta para
...
void row(const char *name, size_t n, Trio t) {
     ...
     * (C contra C++) y donde estamos frente al sistema (libc contra la mejor de
     * las nuestras).  Vienen MEDIDAS vuelta a vuelta, no divididas aqui; el por
     * que esta en `Trio::gain`. */
     ...
     * con nombre.  Aqui el margen es la dispersion observada de esa fila con un
     * suelo del 2%, que es lo mejor que este reloj distingue.  Una fila que se
     * mueve mucho NO puede declarar ganador, y eso es exactamente lo que se
     * quiere: que el veredicto solo aparezca cuando la diferencia es mayor que
     * el ruido que la produce.
     *
    ...
    /* Las mismas dos proporciones y el mismo margen que acaban de decidir el
...
void header(const char *name, const char *what) {
    g_plots.emplace_back();
    g_plots.back().name = name;
    g_plots.back().pass = g_pass;
    report::section(name, what);
...
void core_plots() {
    ...
    /* Las secciones, en el orden de la primera pasada. */
    for (const Plot &first : g_plots) {
        ...
        static const char *kColors[3] = {nullptr, nullptr, nullptr};
        kColors[0] = report::cyan();
        kColors[1] = report::green();
        kColors[2] = report::amber();

        tp.reserve(names.size());
        for (size_t i = 0; i < names.size(); ++i) {
            for (const Plot &q : g_plots) {
                ...
                series.push_back(
                    {names[i], nullptr, kColors[i % 3]}); // el puntero, luego
                break;
            }
        }
...
const char *pass_summary() {
    ...
    for (const PassInfo &p : g_passes) {
        const char *tint = (&p == best) ? report::green()
                                        : (p.floor > 0.05 ? report::amber()
                                                          : report::dim());
 ...
 * por que se dibuja UNA pasada y cual.  Dibujar las tres serian setenta y dos
 * paneles para responder tres veces a la misma pregunta; lo que si cambia entre
 * pasadas tiene su propio bloque, `core_plots`.
 */
...
constexpr int rounds_for(size_t n) {
    return n <= 256 ? 2000000 : (n <= 4096 ? 400000 : (n <= 65536 ? 40000 : 3000));
}
 ...
 * bloques pequenos la copia dura pocos nanosegundos y el reloj pesa mas.  Se
...
#define VESTA_CALIB(N)                                                         \
    {                                                                          \
        const Trio t = hot_trio<N, 1, 3>(rounds_for(N) / 4);                   \
        if (t.gain > 0.0) {                                                    \
...
/* Las filas.  El tamano tiene que ser una CONSTANTE porque la interfaz con
 * tipo necesita un tipo, asi que la lista se expande con un macro en vez de
 * recorrerse con un bucle. */
 ...
 * multiplos en la seccion alineada hacia que las dos columnas copiaran
 ...
 *
 * Asi que la seccion alineada lleva solo multiplos de 32, y los tamanos raros
 * van a la seccion sin alinear, donde el tipo mide lo que dice. */
 ...
 * de camino -- la cascada sin bucle, el bucle base y el ancho -- y mirar solo
 * los extremos deja los bordes sin probar, que es donde falla este codigo. */
...
/* Multiplos de 16 que NO lo son de 32, con un tipo alineado a 16.  Existen
 * porque son los que caen JUSTO entre dos vueltas del bucle ancho, y con la
 * lista de 32 no se prueba ninguno. */
#define VESTA_ALIGNED16_SIZES(X)                                               \
    X(16) X(48) X(80) X(112) X(144) X(176) X(208) X(240) X(272) X(496)         \
        X(1008) X(2032) X(4080)

/* Multiplos de 64: el tipo ocupa lineas de cache ENTERAS, asi que ninguna
 * escritura puede partir una linea ni con la peor suerte.  Es la cota superior
 * de lo que la alineacion puede dar. */
#define VESTA_ALIGNED64_SIZES(X)                                               \
    X(64) X(128) X(192) X(256) X(384) X(512) X(1024) X(2048) X(4096) X(16384)  \
        X(65536)

/* Multiplos de 8 que no lo son de 16, con un tipo alineado a 8.  Este caso
 * importa por lo contrario que los otros: la alineacion se SABE pero NO BASTA
 * -- 8 es menos que los 16 del vector --, asi que el prologo se sigue
 * emitiendo.  Es el camino que comprueba que saber de menos no rompe nada. */
#define VESTA_ALIGNED8_SIZES(X)                                                \
    X(8) X(24) X(40) X(56) X(88) X(120) X(152) X(248) X(504) X(1016) X(4088)

/* Cualquier tamano, con alineacion natural.  Incluye los de una unidad mas y
 * una menos que cada frontera (15/16/17, 31/32/33, 127/128/129, ...): ahi es
 * donde un `>=` mal puesto cambia de camino y donde la cola se come o se deja
 * un byte. */
...
void run_sections() {
    header("aligned 32",
           "aligned to 32 -- where the type can drop the alignment prologue; "
           "sizes must be a multiple of 32, see the note on the size lists");
#define ROW_A32(N) row(N, hot_trio<N, 32, 0>(rounds_for(N)));
...
#define ROW_A64(N) row(N, hot_trio<N, 64, 0>(rounds_for(N)));
...
#define ROW_A16(N) row(N, hot_trio<N, 16, 0>(rounds_for(N)));
...
#define ROW_A8(N) row(N, hot_trio<N, 8, 0>(rounds_for(N)));
...
#define ROW_ANY(N) row(N, hot_trio<N, 1, 3>(rounds_for(N)));
    VESTA_ANY_SIZES(ROW_ANY)
#undef ROW_ANY

    header("scattered", "scattered over 16 MiB, aligned -- not in cache");
#define ROW_COLD(N) row(N, scattered_trio<N, 32, 0>(rounds_for(N) / 8));
...
#undef ROW_COLD
     ...
     * la lista, asi que se ve como escala con el tamano pero no con que le pasa
     * a UN tamano cuando la direccion se mueve.  Son dos ejes y hacen falta los
     * dos. */
...
#define ROW_BY_ALIGN(N, A) row(#N " @" #A, N, hot_trio<N, A, 0>(rounds_for(N)));
    ROW_BY_ALIGN(256, 32)
    ROW_BY_ALIGN(256, 16)
    ROW_BY_ALIGN(256, 8)
    ROW_BY_ALIGN(256, 1)
...
#define ROW_BY_OFF(N, O) row(#N " +" #O, N, hot_trio<N, 1, O>(rounds_for(N)));
    ROW_BY_OFF(256, 0)
    ROW_BY_OFF(256, 1)
    ROW_BY_OFF(256, 2)
    ROW_BY_OFF(256, 4)
...
int main() {
    std::printf("== vesta_memcpy: the C interface, the typed C++ one, and "
                "libc ==\n\n");
    std::printf("All three are called in this process on the same addresses,\n"
                "interleaved A-B-C-C-B-A, in nanoseconds per copy.  `C/C++` is\n"
